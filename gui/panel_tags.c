...
// void app_panel_tags_set_notes_cb(GtkListBoxRow* row)
{
    // Show notes for selected tag
    int index = gtk_list_box_row_get_index(row);
    g_print("%d\n", index);
    Tag tag = user_data->tags[index];
    g_print("%s\n", tag.name);
}
...
GtkWidget *app_panel_tags_create(Content *notes_content)
{
    ...
    // g_signal_connect(list, "row-activated", G_CALLBACK(app_panel_tags_set_notes_cb), NULL);
